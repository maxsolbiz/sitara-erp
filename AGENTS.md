...
- Item 1.5 mechanical sweep DONE (redone TWICE: first pass missed single-quoted object literals, second added backtick nav forms — nav 20→29): 92 routes, 152 extracted targets, 84 unique, 83 matched, 1 unmatched (forms lit=41 obj=59 tpl=23 nav=29). Sole dead static link: `sales/page.tsx:38` → `/sales/returns/create/:id` (page reads `?saleId=`; fixed in Item 1.6 via link change + mount-only `useEffect`). Backend-generated links (search `link:`, notification `link:`) all resolve. Query-param deep-links (`/expenses?status=`, `/sales/returns?status=`) land on pages that IGNORE the param (client-side filter only) — cosmetic wrinkle, noted
