...
export default function PrintLedgerPage() {
  ...
  const load = useCallback(async (start: string, end: string, type: string) => {
    setLoaded(false);
    let url = `/customers/${params.id}/ledger?perPage=1000`;
  ...
  // Inject print styles
  useEffect(() => {
    const styleId = 'print-ledger-style';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="screen-only">
        <PageHeader title="Customer Ledger" description={customer?.fullName ? `Ledger for ${customer.fullName}` : 'Loading...'}>
      ...
      <div className="statement">
        <div className="text-center border-b-2 border-gray-900 pb-3 mb-5 print-header-wrap">
            ...
            <h2 className="text-lg font-semibold tracking-wide print-header-title">CUSTOMER LEDGER</h2>
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm mb-5 border rounded p-3 bg-muted/30 print-info-box">
          ...
          <div><span className="font-semibold">Balance:</span> <span className="font-bold">{formatPkr(customer.currentBalance)}</span></div>
        </div>

        {/* Summary cards — internal ledger metrics */}
        <div className="grid grid-cols-4 gap-3 mb-5 print-cards-wrap">
          <div className="rounded-lg p-3 text-center border bg-card print-card">
            <p className="text-xs text-muted-foreground print-card-label">Current Balance</p>
            <p className="text-lg font-bold print-card-value">{formatPkr(customer.currentBalance)}</p>
          </div>
          <div className="rounded-lg p-3 text-center bg-gradient-red print-card">
            <p className="text-xs opacity-80 print-card-label">Total Sales</p>
          ...
          <div className="rounded-lg p-3 text-center bg-gradient-indigo print-card">
            <p className="text-xs opacity-80 print-card-label">Credit Limit</p>
            <p className="text-lg font-bold print-card-value">{customer.creditLimit ? formatPkr(customer.creditLimit) : 'No Limit'}</p>
          </div>
        </div>
              ...
              <tr key={e.id} className={i < reversed.length - 1 ? 'border-b border-border/50' : ''}>
                ...
                <td className="p-2">
                      <span style={{
                        ...
                        background: ['SALE','VOID','ADJUSTMENT','MIGRATION'].includes(e.type) ? '#fee2e2' : '#dcfce7',
                        color: ['SALE','VOID','ADJUSTMENT','MIGRATION'].includes(e.type) ? '#991b1b' : '#166534',
                      }}>
                        {e.type}
                      </span>
                    </td>
        ...
        <div className="flex justify-between items-center text-xs text-muted-foreground mt-6 pt-4 border-t border-border print-footer">
            <span className="print-footer-left">
              Customer Ledger &mdash; {customer.fullName} &mdash; {customer.customerCode}
            </span>
