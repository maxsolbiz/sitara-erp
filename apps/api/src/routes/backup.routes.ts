...
router.get('/', rbacMiddleware('settings.view'), async (req: Request, res: Response) => {
  try {
    ...
    const lastCompleted = await prisma.backupRecord.findFirst({ where: { status: 'completed' }, orderBy: { createdAt: 'desc' } });
...
router.get('/:id/validate', rbacMiddleware('settings.backup'), async (req: Request, res: Response) => {
  try {
    const result = await validateBackup(parseInt(req.params.id));
...
router.post('/:id/restore/confirm', rbacMiddleware('settings.backup'), async (req: Request, res: Response) => {
  try {
    ...
    if (!confirmToken) { res.status(400).json({ success: false, error: 'confirmToken required' }); return; }
    const result = await executeRestore({ backupId: parseInt(req.params.id), confirmToken, restoredBy: req.user ? BigInt(req.user.userId) : undefined });
    res.json({ success: true, data: result });
  } catch (error: any) { res.status(500).json({ success: false, error: error.message }); }
});
